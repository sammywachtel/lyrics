...
test.describe('Core App Functionality', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for app to be ready (look for main content)
    await page.waitForSelector('[data-testid="app-content"], .min-h-screen', { timeout: 10000 });
  });
...
test.describe('Song List Functionality', () => {
  ...
  test('Song list loads and displays content', async ({ page }) => {
    // Look for song list indicators (either songs or empty state)
    ...
    if (hasEmptyState) {
      ...
      await expect(page.locator('text=Create Your First Song, text=Create New Song').first()).toBeVisible();
    }
  });

  test('Create new song button works', async ({ page }) => {
    // Look for create button (either in header or empty state)
    const createButton = page.locator('text=Create New Song, text=Create Your First Song').first();
  ...
  test('Song statistics display correctly', async ({ page }) => {
    ...
    const hasStats = await page.locator('text=/\\d+ songs in/', 'text=/\\d+ song/', 'text=songs in your').isVisible();
    const hasEmptyState = await page.locator('text=Your creative journey starts here').isVisible();
...
test.describe('Song Editor Functionality', () => {
  ...
  test('Editor loads when accessing a song', async ({ page }) => {
    ...
    const editButtons = page.locator('[data-testid*="edit"], text=Edit, .edit-button');
    ...
    if (editButtonCount > 0) {
      ...
      await expect(page.locator('.rich-text-editor, [contenteditable], .editor-container')).toBeVisible({ timeout: 10000 });

      // Should show editor UI elements
      const hasEditorUI = await page.locator('.prosody-line, .section-border, .editor-toolbar').count() > 0;
      expect(hasEditorUI).toBeTruthy();
    } else {
      // Skip if no songs to edit
      test.skip();
    }
  });
  ...
  test('Editor shows basic functionality', async ({ page }) => {
    ...
    const editButtons = page.locator('[data-testid*="edit"], text=Edit');
    const editButtonCount = await editButtons.count();

    if (editButtonCount > 0) {
      await editButtons.first().click();
      await page.waitForSelector('.rich-text-editor, [contenteditable]', { timeout: 10000 });

      // Check for word count or statistics
      const hasStats = await page.locator('text=/Words: \\d+/, text=/Lines: \\d+/, text=/Characters: \\d+/').isVisible();
      expect(hasStats).toBeTruthy();

      // Check for section functionality (if present)
      const hasSections = await page.locator('text=/Sections: \\d+/, .section-border, .section-tag').count() > 0;
...
test.describe('API and State Management', () => {
  ...
  test('Redux store is functioning', async ({ page }) => {
    ...
    // Check that Redux DevTools can access the store (if extension is available)
    const hasReduxStore = await page.evaluate(() => {
      return typeof window.__REDUX_DEVTOOLS_EXTENSION__ !== 'undefined' ||
             typeof window.store !== 'undefined' ||
             document.querySelector('[data-reactroot]') !== null;
    });

    // At minimum, React should be working (indicates Redux Provider is likely working)
    expect(hasReduxStore).toBeTruthy();
  });
  ...
  test('API calls complete without hanging', async ({ page }) => {
    ...
    const hasInfiniteLoading = await page.locator('.animate-spin, .loading, [data-testid="loading"]').isVisible();

    // If loading indicators are present, they should disappear within reasonable time
    if (hasInfiniteLoading) {
      await expect(page.locator('.animate-spin, .loading').first()).not.toBeVisible({ timeout: 15000 });
    }
  });
...
test.describe('Error Handling', () => {
  ...
  test('App shows appropriate loading states', async ({ page }) => {
    ...
    const hasLoadingState = await page.locator('.animate-spin, .loading, text=Loading').isVisible({ timeout: 1000 });
