...
test.describe('Redux State Management', () => {
  ...
  test('RTK Query caching works on navigation', async ({ page }) => {
    ...
    if (editButtonCount > 0) {
      ...
      const backButton = page.locator('text=Back, text=← Back, [aria-label="Back"], .back-button').first();
  ...
  test('Auto-save functionality works', async ({ page }) => {
    ...
    if (editButtonCount > 0) {
      ...
      const autoSaveIndicators = page.locator('text=Auto-save, text=Saving, text=Saved, .auto-save');

      // Type some content to trigger auto-save
      const editor = page.locator('[contenteditable], textarea, .rich-text-editor').first();
