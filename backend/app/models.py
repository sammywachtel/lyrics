...
class NarrativeSettings(BaseModel):
    ...
    why_message: Optional[str] = Field(None, description="Why is this message important?")
    point_of_view: NarrativePOV = Field(NarrativePOV.FIRST_PERSON, description="Narrative perspective")
...
class StructureSettings(BaseModel):
    ...
    story_boxes: List[StructuralBox] = Field(default_factory=list, description="Story progression boxes")
    section_structure: List[SectionStructure] = Field(default_factory=list, description="Section organization")
    target_duration_minutes: Optional[float] = Field(None, ge=0.5, le=15.0, description="Target song duration")
    overall_energy_arc: Optional[str] = Field(None, description="Energy progression throughout song")
...
class SoundSettings(BaseModel):
    ...
    prosody_settings: ProsodySettings = Field(default_factory=ProsodySettings, description="Rhythm and meter")
    syllable_emphasis: bool = Field(True, description="Consider syllable stress patterns")
    rhythmic_consistency: int = Field(5, ge=1, le=10, description="How consistent should rhythm be")


class StyleSettings(BaseModel):
    """Style tab - Genre and artistic direction."""
    primary_genre: Optional[str] = Field(None, description="Primary musical genre")
    sub_genres: List[str] = Field(default_factory=list, description="Additional genre influences")
    artist_references: List[str] = Field(default_factory=list, description="Artists to emulate")
    innovation_level: int = Field(5, ge=1, le=10, description="Innovation vs traditional balance")
    avoid_cliches: bool = Field(True, description="Avoid common genre cliches")
...
class ContentSettings(BaseModel):
    ...
    primary_keywords: List[str] = Field(default_factory=list, description="Key words to include")
    metaphor_themes: List[str] = Field(default_factory=list, description="Metaphorical themes")
    avoid_words: List[str] = Field(default_factory=list, description="Words to avoid")
    synonym_groups: Dict[str, List[str]] = Field(default_factory=dict, description="Synonym alternatives")
    content_guidelines: Optional[str] = Field(None, description="Additional content guidance")


class AISettings(BaseModel):
    """AI tab - AI assistance configuration."""
    creativity_level: int = Field(5, ge=1, le=10, description="AI creativity level")
    preserve_user_phrases: bool = Field(True, description="Preserve original user phrases")
    ...
    writing_style_adaptation: bool = Field(True, description="Adapt to user's writing style")
...
class SongSettings(BaseModel):
    ...
    foundation: NarrativeSettings = Field(default_factory=NarrativeSettings, description="Foundation/narrative settings")
    
    # Structure tab - Song organization  
    structure: StructureSettings = Field(default_factory=StructureSettings, description="Structure settings")
    
    # Sound tab - Rhyme and prosody
    sound: SoundSettings = Field(default_factory=SoundSettings, description="Sound/prosody settings")
    
    # Style tab - Genre and artistic direction
    style: StyleSettings = Field(default_factory=StyleSettings, description="Style and genre settings")
    
    # Content tab - Keywords and content guidelines
    content: ContentSettings = Field(default_factory=ContentSettings, description="Content management settings")
    
    # AI tab - AI assistance configuration
    ai: AISettings = Field(default_factory=AISettings, description="AI assistance settings")
    
    # Backwards compatibility fields (deprecated but maintained)
    narrative_pov: Optional[NarrativePOV] = Field(None, description="[Deprecated] Use foundation.point_of_view")
    central_theme: Optional[str] = Field(None, description="[Deprecated] Use foundation.central_theme")
    six_best_friends: Optional[SixBestFriends] = Field(None, description="[Deprecated] Use foundation.six_best_friends")
    structural_boxes: Optional[List[StructuralBox]] = Field(None, description="[Deprecated] Use structure.story_boxes")
    section_structure: Optional[List[SectionStructure]] = Field(None, description="[Deprecated] Use structure.section_structure")
    rhyme_preferences: Optional[RhymePreferences] = Field(None, description="[Deprecated] Use sound.rhyme_preferences")
    prosody_settings: Optional[ProsodySettings] = Field(None, description="[Deprecated] Use sound.prosody_settings")
    keyword_settings: Optional[KeywordSettings] = Field(None, description="[Deprecated] Use content settings")
    style_guide: Optional[StyleGuide] = Field(None, description="[Deprecated] Use style settings")
    target_duration_minutes: Optional[float] = Field(None, description="[Deprecated] Use structure.target_duration_minutes")
    overall_mood: Optional[str] = Field(None, description="[Deprecated] Use style.overall_mood")
    energy_level: Optional[int] = Field(None, description="[Deprecated] Use style.energy_level")
    ai_creativity_level: Optional[int] = Field(None, description="[Deprecated] Use ai.creativity_level")
    preserve_user_phrases: Optional[bool] = Field(None, description="[Deprecated] Use ai.preserve_user_phrases")
    auto_suggestions: Optional[bool] = Field(None, description="[Deprecated] Use ai.auto_suggestions")
    ...
    def validate_structure(cls, v: StructureSettings) -> StructureSettings:
        ...
        if v.target_duration_minutes is not None and (v.target_duration_minutes < 0.5 or v.target_duration_minutes > 15.0):
...
class ProsodyConfig(BaseModel):
    ...
    custom_meter_patterns: List[str] = Field(default_factory=list, description="Custom meter patterns")
    stress_patterns: Dict[str, List[int]] = Field(default_factory=dict, description="Stress patterns by section")
    syllable_targets: Dict[str, int] = Field(default_factory=dict, description="Syllable targets by section")
    rhythm_constraints: Dict[str, Any] = Field(default_factory=dict, description="Additional rhythm constraints")
    ...
    def validate_meter_patterns(cls, v: List[str]) -> List[str]:
        ...
        for pattern in v:
            if not valid_pattern.match(pattern):
                raise ValueError(f'Invalid meter pattern: {pattern}. Use only "/", "-", and "u" characters')
        return v
...
class SongVersion(BaseModel):
    ...
    settings: SongSettings = Field(default_factory=lambda: SongSettings(), description="Settings at this version")
    prosody_config: ProsodyConfig = Field(default_factory=ProsodyConfig, description="Prosody config at this version")
...
class SongSettingsHistory(BaseModel):
    ...
    settings_before: Dict[str, Any] = Field(default_factory=dict, description="Settings before change")
    settings_after: Dict[str, Any] = Field(default_factory=dict, description="Settings after change")
    prosody_config_before: Dict[str, Any] = Field(default_factory=dict, description="Prosody config before change")
    prosody_config_after: Dict[str, Any] = Field(default_factory=dict, description="Prosody config after change")
...
class ProsodyConfigUpdate(BaseModel):
    ...
    prosody_config: ProsodyConfig = Field(..., description="Updated prosody configuration")
